...
**TWC computation:** Total Wellness Coherence is a composite of all 8 dimension scores, weighted by role configuration. Role-specific weight overrides are permitted (e.g., a research agent may weight Intellectual higher).

> See premium tier for advanced scoring.

---

## 2b. Advanced Scoring Model

The 8D framework uses cross-dimensional interaction modeling to capture how wellness dimensions affect each other. This produces more accurate assessments than simple averaging.
...
The 8D system detects when degradation in one dimension triggers degradation in others, enabling targeted interventions at the root cause rather than surface symptoms.
...
Fleet cascade detection catches when one agent's failure propagates to other agents. Critical infrastructure agents are monitored for blast radius impact.
...
| Metric | Definition |
|--------|-----------|
| TWC | Total Wellness Coherence: composite of all 8 dimension scores with cross-dimensional weighting. See premium tier for advanced scoring. |
...
| 1.1.0 | 2026-03-22 | Health Observer Agent Cycle 1 review. Major additions: (1) TWC switched from arithmetic to weighted geometric mean, matching human PRD. (2) Temporal smoothing for score stability. (3) Pre-assessment operational state marker (analog to human mood marker). (4) Operational Consistency Index (OCI), analog to human CSI. (5) Dimensional Coherence Score. (6) Score confidence levels. (7) Long-context degradation protocol. (8) Agent identity erosion detection. (9) Hallucination as cross-dimensional health signal. (10) Multi-model agent health guidance. (11) Graceful Degradation Protocol with Three Laws of Degradation. (12) Worked example for composite score computation. (13) Cross-dimensional cascade detection. (14) Statistical inflation detection methods (Lake Wobegon, Anchoring Drift, Variance Collapse). (15) Expanded human-AI correlation map with 10 new entries. (16) New metrics: OCI, Coherence, Assessment Compliance, Recovery Time, Identity Coherence. |
