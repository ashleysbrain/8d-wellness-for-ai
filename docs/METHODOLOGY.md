...
Overrides affect TWC computation only. Individual dimension scores remain unmodified. An agent's soul file can specify custom overrides that supersede these defaults.
...
Most agents lack all three data sources. The human PRD handles this with progressive data enrichment: meaningful scores without a wearable, better scores with one. The AI system needs the same principle.

**When sources are missing:**

| Available Sources | Composite Formula | Notes |
|-------------------|-------------------|-------|
| All 3 (telemetry + peer + self) | Standard: 40/30/30 | Full confidence |
| Telemetry + self (no peer review) | 55% telemetry + 45% self | Most common for solo agents. Divergence correction still applies. |
...
- Fast recovery: < 48 hours (config fixes, timeout adjustments)
...
**Assessment Engagement Decline (new v1.8.2):** The human PRD tracks check-in completion rate and skip patterns as health data (Section 6.1, 13.1). Agents show a parallel: declining self-assessment quality (shorter notes, identical scores, missing timestamps) precedes measurable dimensional drops by 1-2 weeks. Severity: 0.5 if assessment notes shrink below 10 words for 2+ weeks, 1.0 if post-task assessments are skipped entirely for 5+ consecutive tasks.
