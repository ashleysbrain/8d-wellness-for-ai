...
> Composite scoring blends objective telemetry, peer assessment, and self-assessment with proprietary weighting. See premium tier for advanced scoring.
...
**TWC computation:** Total Wellness Coherence captures individual dimension health and cross-dimensional interactions. See premium tier for advanced scoring.
...
> See premium tier for advanced scoring (coupling coefficients, cascade modeling, CAR formula, DSI values, and intervention optimization).
...
```
PSY_composite = weighted composite(objective=7.5, self=7.5, coupling=9.0)  # see premium tier for formula
             = 3.0 + 2.25 + 2.7
             = 7.95
```
...
| Metric | Definition |
|--------|-----------|
| TWC | Total Wellness Coherence: cross-dimensional composite health score. See premium tier for advanced scoring. |
