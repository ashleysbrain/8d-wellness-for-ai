...
TWC goes beyond simple averaging. It captures cross-dimensional coupling — the way disruption in one dimension cascades through others. Dimensions interact: a change in one creates downstream effects. The interaction term typically accounts for 30-50% of true wellness variance and is what makes the framework predictive, not just descriptive.

See premium tier for advanced scoring — including the full TWC formula, coupling coefficient matrix, CAR formula, layer weights, and cascade modeling.

### 3.1b Coupling Coefficient Matrix

Dimensions interact with varying strength. The coupling matrix quantifies these relationships across all 8 dimension pairs. The Dimension Sensitivity Index (DSI) identifies which dimensions act as hubs.

See premium tier for advanced scoring — including the full coupling matrix, coefficient values, DSI rankings, and cross-dimensional coupling formula.
...
See premium tier for advanced scoring — including layer weights, the D_coupled formula, and the full final score calculation.
...
The coupling matrix reveals where to intervene for maximum positive cascade. Not all dimensions are equal leverage points. Some act as hubs — improving them triggers positive cascades across multiple others.

See premium tier for advanced scoring — including the Intervention Leverage Score (ILS) formula, cascade intervention patterns with specific coefficients, and Minimum Effective Intervention (MEI) calculations.
