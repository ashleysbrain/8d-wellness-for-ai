...
```
D_final(i) = weighted composite of objective, peer, and self scores (see premium tier for advanced scoring)
```
...
TWC captures cross-dimensional interactions and provides a predictive measure of overall wellness health. See premium tier for advanced scoring (full formula, coupling coefficients, CAR, and DSI values).
...
```
_Intervention leverage scoring uses proprietary coupling mathematics. See premium tier for advanced scoring._

**Top cascade intervention patterns:**

| Pattern | Root Signal | Primary Target | Expected Cascade |
|---------|-----------|----------------|------------------|
| Infrastructure-Cognitive Spiral | PHY + PSY declining | Physical (stabilize infra) | PHY → PSY cascade. See premium tier for advanced scoring. |
| Performance-Cost Decline | VOC + FIN declining | Vocational (small wins) | VOC cascade through SPI + INT + FIN. See premium tier for advanced scoring. |
| Collaboration Breakdown | SOC dropping | Social (handoff quality) | SOC → PSY + SPI cascade. See premium tier for advanced scoring. |
| Full-System Decline (3+ dims) | Multiple dims below threshold | Psychological (hub dimension) | Broadest cascade. See premium tier for advanced scoring. |

**Minimum Effective Intervention (MEI):** Target the dimension with the highest intervention leverage for maximum cascade benefit. See premium tier for advanced scoring.
...
| Three-Source Health Score | Multi-source composite (see premium tier for advanced scoring) | Algorithm specification |
